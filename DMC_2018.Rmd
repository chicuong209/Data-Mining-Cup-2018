...
```
### Error is quite large because in our data set has nominal attributes (color and brand). Linear regression is the simplest model but it's useful for data set which contains only numerical attributes. 
...
```
### As we can see, the stakcing method with XGBOOST model returns the best of result ERROR. So i decided to choose stacking method for training data.I chose stacking model with XGBOOST for final prediction.
